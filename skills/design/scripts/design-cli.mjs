...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--host') {
      host = argv[++index];
      if (!host) throw new Error('--host requires agent-plugin, cursor, or codex.');
...
export function runDesignCli(argv = process.argv.slice(2)) {
  const { command, action, flags, host } = parseArgs(argv);
  ...
  if (command === 'setup') {
    return print(setupProject(projectRoot, { ...options, apply: flags.has('--apply'), enableHook: !flags.has('--without-hook') }));
  }
  if (command === 'hook' && ['on', 'off'].includes(action)) {
    return print({ written: setProjectHook(projectRoot, action === 'on', options), enabled: action === 'on', host });
  }
