...
const HOOK_MANIFEST_TARGETS = [
  ...
  {
    provider: '.cursor',
    skillRel: '${CURSOR_PLUGIN_ROOT}/skills/impeccable',
    ...
    manifest: () => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: 'node "${CURSOR_PLUGIN_ROOT}/skills/impeccable/scripts/hook-before-edit.mjs"',
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
...
function statusReport(cwd) {
  ...
  const lines = [
    ...
    `  state:        ${explicitEnabled ? 'enabled' : 'disabled'}`,
