...
```bash
<IMPECCABLE_SKILL_ROOT>/scripts/impeccable critique-storage latest "<resolved target>" --json
```
...
```bash
<IMPECCABLE_SKILL_ROOT>/scripts/impeccable critique-storage close "<resolved target>" "<snapshot_file returned by latest>"
```
