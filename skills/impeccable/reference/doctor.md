...
- **Tool version.** The installed skill is older than the published one. `context.mjs` reports that at boot as `UPDATE_AVAILABLE` and `/design doctor` fixes it. Not this command's job.
...
```
node "${CURSOR_PLUGIN_ROOT}/skills/impeccable/scripts/doctor.mjs" --json
```
...
- **`auto`** carries no decision. Run `node "${CURSOR_PLUGIN_ROOT}/skills/impeccable/scripts/doctor.mjs" --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
