...
check(manifest.version === '0.9.0', 'Cursor manifest version must be 0.9.0.');
...
check(modules.length === 2, `Version 0.9.0 must contain exactly design-core and impeccable modules, found ${modules.length}.`);
...
for (const module of modules) {
  ...
  check(module.contributes.mcpServers.length === 0, `${module.id} must not contribute MCP in 0.9.0.`);
}
