...
The Design lifecycle and read-only change review remain available after a client loads the skill, but its hook state is `unavailable` and hook mutation is refused. Change review uses the bundled JSON scope resolver and describes follow-up operations through the loaded `design` skill without inventing a universal command syntax. Impeccable follows the matching bundled `reference/degraded/` role contract inline for asset production, documentation, finish review, and live manual apply because Agent Plugins v1 does not standardize native subagents.
