...
## Command selection and reference maintenance

Design selects implicit operations directly from the installed Impeccable skill's current command table, aliases, routing guidance, and linked playbooks. The generated capability index defines the selection procedure without copying Impeccable command names or maintaining keyword aliases. Candidate application and offline sync replace that skill inventory, so added, removed, renamed, and redefined operations become the selection source together. Target builds preserve the same source for Cursor, Codex, and Agent Plugins (where the table is called Operations).

Design-core operation intents live beside their command triggers in the module registry. Index generation requires exactly one nonempty description per trigger; candidate preparation and offline sync validate that mapping before writing. Natural-language routing does not loosen write approvals or detector target requirements.

The shared readiness assessment consumes the bundled read-only doctor through the existing runtime projection. Preserve its project/app resolution, inherited context paths, maintenance findings and failure reporting across engine updates. Unknown report formats must remain unverified, not ready. Test healthy context with disabled hooks, missing context, migrations, unavailable engines, and selected child apps; never derive setup completeness from hook activation or add runtime self-update.

Preserve diagnostic completeness signals outside the findings array when normalizing upstream reports. An empty findings array does not prove that every check ran: `ruleRegistryAvailable: false` means ignored rule IDs could not be validated. A missing or invalid availability value is also unknown, not success. Report the incomplete check and keep readiness unverified while retaining independently resolved app scope and canonical context. Continue permitted UI work without automatic repair or broader write authority. The CLI regression in [project-state tests](../tests/project-state.test.mjs) exercises available, unavailable, missing and invalid registry signals through status, diagnose and setup preview, plus confirmed setup readback. Preserve those cases when adapting to an engine update.

[The command reference](commands.md) is generated by `npm run build:commands` from the bundled skill's Commands table, with its command-metadata inventory cross-checked and its local references verified. The approved pin supplies the version. `npm run check:commands`, included in `release-check`, rejects stale output without rewriting it. Unknown table formats or mismatched inventories require a reviewed generator adjustment.

Candidate preparation and offline sync use the same generators for the routing index and command reference against the projected skill and pin. Both validate before destination writes; offline preview remains read-only. The reference and routing index belong to `candidateDestinations`, so their diff, baseline, output integrity, application, and rollback share the existing candidate transaction. Do not copy the currently checked-in reference into a new candidate.

For each update, review added, removed, renamed, and deprecated commands as well as changed behavior, prerequisites, and plugin-specific constraints in the command playbooks. Recheck implicit selection with representative natural-language requests, explicit-command precedence, assessment versus editing, and setup boundaries. Update the selection procedure if upstream routing semantics change, and the generator's explanatory sections and usage guide where needed. Generated inventory checks establish source freshness, not semantic routing quality or fresh host behavior. Review `doctor` and hooks especially: Impeccable retains `doctor`, while Design integration uses `diagnose`. Preserve first-party English guidance and all host boundaries.
...
Run `npm run verify:impeccable-engine` for the [repository verifier](../.agents/skills/verify-impeccable-engine/SKILL.md). The focused command remains available for candidate checks. `release-check` uses its `--all` mode to discover and execute every test file once. Both modes store test selection, source hashes, results, cleanup and platform evidence outside the checkout, and report unexecuted platforms. CI exercises the native engine and adapters on five operating-system/architecture runners. Fresh Cursor/Codex smoke remains separately commissioned; neither CI nor local adapter fixtures prove editor activation.
