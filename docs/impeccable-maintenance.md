...
Candidate preparation and offline sync use the same generator against the projected skill and candidate pin. The reference belongs to `candidateDestinations`, so its diff, baseline, output integrity, application, and rollback share the existing update transaction. Do not copy the currently checked-in reference into a new candidate.

For each update, review added, removed, renamed, and deprecated commands as well as changed behavior, prerequisites, and plugin-specific constraints in the command playbooks. Update the generator's explanatory sections and the usage guide where needed. Generated inventory checks cannot establish that a human-facing explanation still reflects a changed playbook. Review `doctor` and hooks especially: Impeccable retains `doctor`, while Design integration uses `diagnose`. Preserve first-party English guidance and all host boundaries.
