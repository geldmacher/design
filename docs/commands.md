...
| How should this interface move? | `animate` for animation direction; `motion` for technical implementation and Motion APIs. |
...
| Entry point | Cursor | Codex |
| --- | --- | --- |
| Through Design | `/design motion <request>` | `$design motion <request>` |
| Direct specialist | `/motion <request>` | `$motion <request>` |

Motion covers CSS, JavaScript, React, Vue, Base UI and Radix best practices, animation implementation and Motion API questions. Design also selects it for clear Motion-specific natural-language requests. General animation direction stays with Impeccable `animate`; explicit Impeccable requests keep their meaning. Advice and assessment remain read-only.

For example: `$design motion Fix AnimatePresence in app/cart using the installed version and preserve reduced-motion behavior`. For reference-only work: `$design motion Find and read a free React exit-animation example; do not edit files`.

Local guidance needs only the loaded plugin. Official documentation search and anonymously readable examples additionally need the bundled Motion MCP connection, network access and the corresponding host search and resource-reading tools. No Motion account or token is required. Missing or blocked online tools fall back to pinned local guidance with an explicit freshness limitation.

The plugin does not install project dependencies or migrate `framer-motion`. Motion+, MotionScore, CSS easing generation, saved transitions and the transition editor are outside its scope. `setup` and optional hooks are independent of the MCP connection; status reports its configuration without testing connectivity.

See [worked Motion examples](usage.md#animate-with-motion), [host setup and connection checks](installation.md#bundled-free-motion-documentation), and [maintenance boundaries](motion-maintenance.md). The portable package includes local Motion guidance without an MCP configuration; its client determines invocation syntax.
...
| `motion <request>` | Use the bundled Motion implementation specialist and available free documentation. |
