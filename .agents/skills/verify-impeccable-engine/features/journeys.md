...
The Design CLI's status, diagnose and setup preview share the additive readiness assessment. `tests/project-state.test.mjs` drives the actual CLI in isolated projects and homes: missing context yields attention, current context with disabled hooks remains ready, local overrides are honored, preview is read-only, confirmed setup is reread, malformed configuration is unverified and cannot be overwritten, and migration findings remain proposals. Selected child apps inherit engine-resolved root context; ambiguous workspace setup writes nothing. Compare project and home snapshots before and after read-only journeys.

`tests/portable-runtime.test.mjs` exercises that same contract in all three built packages, including a temporarily unavailable bundled engine. Diagnosis must remain visible with `continueWork: true`, context must be unverified, and setup application must be refused while scope cannot be verified. Restore the fixture engine in cleanup. The full suite also checks operation-intent freshness and both update paths. These CLI trials prove the assessment and write boundaries, not that a live assistant selected a command, performed preflight, or suppressed repeated notices; the shared fresh-host smoke covers those separately.
